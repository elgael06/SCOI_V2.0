...
const cambio_barcode = (codigo) => {
    JsBarcode("#barcode", codigo, { format: "codabar", height: 30, fontSize: 12, font: "Arial Black", width: 1 }).render();
}
...
function printDiv() {
    ...
    html2canvas(document.querySelector("#etiqueta")).then(canvas=> {
        var printWin = window.open(canvas.toDataURL('imagejpeg', 1.0), 'Barcode', 'width=340,height=260,,top=200,left=200,toolbars=no,scrollbars=no,status=no,resizable=no');
        printWin.focus();
        printWin.print();
        //printWin.close();
...
const rotar = () => {
    let rotacion = document.querySelector("#etiqueta").style.transform;
    let giro = rotacion == "" ? "rotate(180deg)" : "";
    document.querySelector("#etiqueta").style.transform = giro;
}
...
(function(){
    ...
    const btn_rotar = document.querySelector("#rotar_etiqueta");
    fecha_caducidad();
    ...
    btn_rotar.addEventListener("click", rotar);
