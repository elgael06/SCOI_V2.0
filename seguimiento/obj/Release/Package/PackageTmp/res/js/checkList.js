...
$(document).ready(function () {
    ...
    //deshabilita los input de la modal ckl
    $(".modalckl [type=text]").attr("disabled", true);
...
var OBJ_CUESTIONARIO = {};
var ESTABLECIMIENTOS = [];

//evento botones cuestionarios
function evento_btn_cuestionario() {
    $(".n_c").on("click", function () {
        OBJ_CUESTIONARIO.id = $(this).attr("id"); OBJ_CUESTIONARIO.nombre = $(this).val()
    })
}
function agregar_cuestionario_boton(nombre_cuest) {//pendiente!!!!!!!!!!!!!!!!!!
    //$("#mostrarTabla_areas [type=button]").remove();
    var t = "";
    var se_agrego = false;
    $.each(cuestionarios_Datos, function (index, item) {
        ...
        if (nombre_cuest == item.nom_cuestionario) {
            ...
            t = "<input type='button'  class='n_c' style='width:90%' ondblclick=' click_btn_cuestionarios(" + item.folio + ",\"" + nombre_cuest + "\")' id='" + item.folio + "' value='" + nombre_cuest + "' title='Folio:" + item.folio + "'/>" +
                "<input style='pos' type='button' title='Close' id='btnQuitselec" + item.folio + "' style='border-radius:25px;' onclick='removerCues(" + item.folio + ")' value='X' />";
...
function agregar_cuestionario() {
    ...
    if ($('.mCuestionario').val()) {
        ...
        $.each(respuestas_tabla, function (index,item) {
            ...
            dat.append($("<td class='pregunta'></td>").append("<h5>" + item.pregunta + "</h5>").css({ "width": "65%" }));
            var btnselect = $('<select id="respuesta' + contar + '" style="height:20px"><option value="">Aplica</option></select>'), optionsi = $("<option >SI</option>"), optionno = $("<option >NO</option>"), optionna = $("<option >NA</option>");
            btnselect.append(optionsi, optionno, optionna);
            dat.append(btnselect)
    ...
    $(".modalckl select").on("change", resultadoConteo)
...
function obtener_lista_cuestionarios_por_establecimiento() {
       ...
       $("#mostrarTabla_areas").append(
            $("<input type='button' />").val(item.cuestionario).attr({
                        "onclick": " click_btn_cuestionarios(" + item.id_cuestionario + ",\'" + item.cuestionario + "\')"
                        , "title": "folio:" + item.id_cuestionario
                        , "id": item.id_cuestionario
            }).css({ "width": "90%" }).addClass("n_c")
            ,$("<input type='button' />").val("X").attr({
                        "onclick": " removerCues(" + item.id_cuestionario + ")"
                        , "title": "Borrar:" + item.id_cuestionario
                        , "id": "btnQuitselec" + item.id_cuestionario
            }).css({ "border-radius": "25px", " background-color": "red" })
        );
    });
}
function agregar_establecimiento_selector() {
    //llamamos a la funcion ajax que contieme el arreglo a usar
    ESTABLECIMIENTOS = conexion_ajax("servicios/checkListServ.asmx/buscar_establecimiento")
    $.each(ESTABLECIMIENTOS, function (i, item) {
        $("#establecimientos").append(
            $('<option>').append(
                item.nombre_establecimiento)
                    .val(item.id_establecimiento)
        );
    });//fin forIn
}
//obtener lider por establecimiento
function agregar_lideres() {
    const encargados = conexion_ajax("servicios/checkListServ.asmx/obtener_lideres", { "tienda": "todos" });

    $("#Nom_Encargados *").remove();

    $.each(encargados, (index, item) => {
        if (index === 0) {
            $("#Nom_Encargados").append(
            $("<option >").val("NA").append("Seleccion Encargado")
            .attr({ "title": "Seleccionar" })
         ).on("change", () => {
        ...
        $("#Nom_Encargados").append(
            $("<option >").val(item.nombre).append(item.nombre + " (" + item.puesto + ").")
                .attr({ "title": item.puesto })
             );
    });
}
...
function resultadoConteo() {
    ...
    $(".modalckl select").each(function (index) {
        if ($(this).val() == "SI") {contadorsi++}//fin if
        else if ($(this).val() == "NO") { contadorno++ }//fin if
        else if ($(this).val() == "NA") { contadorna++ }//fin if
    });//fin each
    //asignamos loa cambios a los controles
...
function recorrer_checkbox() {
    ...
    $(".classcuestionario select").each(function (index, dato) {
      if ($(this).val()!="") {
        contador++;
      }  
      total++;
...
function GuardarEnviar(){
    ...
    var url = "servicios/checkListServ.asmx/guardar_datos_2";//direccion del servicio.
    ...
    var arreglo_respuestas = [];
    var arreglo_posicion = [];
    var arreglo_datos_pregunta = [];
    ...
    $('.tablaPreguntas tr').each(function (index) {
        ...
        if ($('#respuesta' + index).val() === "SI") respuesta = 1;
        else if  ($('#respuesta' + index).val()  === "NO") respuesta = 0;
        else if ($('#respuesta' + index).val() === "NA") respuesta = 2;
        else respuesta = 3;
        arreglo_respuestas.push(respuesta);
        arreglo_posicion.push(ordenCuestiones);
        arreglo_datos_pregunta.push(datos_pregunta)
    });//fin de primer each
    ...
    obj = {
        ...
        'criterio': arreglo_posicion,//criterio
        'respuestas': arreglo_respuestas,//respuesta
        'datos_pregunta': arreglo_datos_pregunta,//datos_pregunta
        'observaciones': duracion,//observaciones
        'aplicador': aplicador,//aplicador
        'encargado': $("#Nom_Encargados").val()
    }//fin obj
...
function resultadockl_parcial(zona) {
    ...
    var respuestas = conexion_ajax("servicios/checkListServ.asmx/conexion_reultados_parcial", { 'folio_sucursal': $("#establecimientos").val(), 'fecha': fechaCompleta(), 'zona': zona }) //  
    ...
    $.each(respuestas, function (index, item) {
        ...
        if (item.respuesta == 0) { $('#respuesta' + index).val("NO") }//,$('#no' + index).attr('checked', true),$('#na' + index).attr('checked', false) }
        else if (item.respuesta == 1) { $('#respuesta' + index).val("SI") }//.attr('checked', true), $('#no' + index).attr('checked', false), $('#na' + index).attr('checked', false) }
        else if (item.respuesta == 2) { $('#respuesta' + index).val("NA") }//.attr('checked', false), $('#no' + index).attr('checked', false), $('#na' + index).attr('checked', true) }
    });
}//fin
...
function llenar_contadores_resultado_total_checkl() {
    ...
    var respuesta;
    var totalsi = conexion_ajax("servicios/checkListServ.asmx/resultados_respuestas", { 'folio_sucursal': $('#establecimientos').val(), 'fecha': fechaCompleta(), "respuesta": 1 });
...
function to_pdf_() {
    ...
    $.each(establecimientos, function (index, it) {
        if (parseInt($('#establecimientos').val()) == it.id_establecimiento)
            estab = it.nombre_establecimiento;
    });
    //Establecimiento y fecha
    doc.text('Establecimiento: ' + estab + ".", 20, 105);

    $.each(retorno_nombres, function (j, item) {
        
        $.each(cuestionarios, function (index, itm) {

            if (itm.folio == item.zona_inspeccion) { nombre = itm.nom_cuestionario }
        });
    ...
    doc.save('CheckList-' + estab + '-' + $('#divDia').val() + '.pdf')
