...
class EstadoDeResultados extends React.Component {
    ...
    grafica_barras($lista_establecimientos) {
        const $lista_conceptos          = ["VENTAS NETAS", "COSTO DE VENTAS", "UTILIDAD EN OPERACION", "GASTOS DE OPERACION", "UTILIDAD NETA OPERACIONAL", "IMPUESTOS ISR", "IMPUESTOS PTU 10%", "GASTOS FAMILIA IZABAL", "UTILIDAD NETA"];
        ...
        $lista_conceptos.forEach((concepto_, p) => {
            ...
            if (concepto_ != "GASTOS FAMILIA IZABAL") {
                const $obj = {
                    label: concepto_,
                    data: [],
...
const VistaTablaPrincipal = ({ lista, evento, Totales, eventoEstablecimiento}) => {
    ...
    const lista_conceptos = ["VENTAS NETAS", "COSTO DE VENTAS", "UTILIDAD EN OPERACION", "TRUPUT DE OPERACION", "GASTOS DE OPERACION", "UTILIDAD NETA OPERACIONAL", "TRUPUT NETA OPERACIONAL", "IMPUESTOS ISR","IMPUESTOS PTU 10%", "GASTOS FAMILIA IZABAL", "UTILIDAD NETA","TRUPUT NETA"];
...
const ModalConceptos = ({ concepto, lista,eventoEstablecimiento }) => {
    ...
    const recorrer_lista = (funcion) => {
        /*
         ...
         resive como parametro funcion que recibe e.
         */
        lista.forEach((establecimiento) => {
            const conceptos = establecimiento.lista_conceptos.filter(e => e.concepto == concepto);
    ...
    const obtener_subclasificadores = (clasificador) => {
        var $lista = [];
        const $lista_subclasificadores = [];
        recorrer_lista(clasificador_ => {
            if (clasificador_.clasificador==clasificador) {
                $lista = $lista.concat(clasificador_.Lista_subclasificadores)
            }
        });
        $lista.forEach(sub => {
            const $index = $lista_subclasificadores.findIndex(e => e == sub.subclacificador);
            if ($index == -1) {
                $lista_subclasificadores.push(sub.subclacificador);
            }
        });
    ...
    const MenuSubclasificadores = ({ clasificador, nombre }) => {
        ...
        return $lista_subclasificadores.map(sub => {
            let ident = crear_identificador(nombre, clasificador);
            return (
                <tr style={{ background: "#e6f0ff",display:"none" }} key={clasificador + "_" + sub} className={nombre} >
                    <th>
    ...
    ///<-----------------------------------------------------------------
    const CrearMovimientos = (subClasificador) => {
        console.log("subClasificador=>", subClasificador);
        recorrer_lista((clasificador, establecimiento, conceptos) => {
    ...
    const CrearSubclasificadores = ({ clasificador, nombre }) => {
        const lista_ = [];
        const $lista_subclasificadores = obtener_subclasificadores(clasificador);
        $lista_subclasificadores.forEach(sub => {
            ...
            recorrer_lista((clasificador_, establecimiento_) => {
                ...
                if (clasificador == clasificador_.clasificador) {
                    ...
                    clasificador_.Lista_subclasificadores.forEach(e => {
                        if (e.subclacificador == sub) {
                            lista_auxiliar[pos_est] = e.Total_Costo;
                            total_sub += redondeo(e.Total_Costo);
                            CrearMovimientos(e.subclacificador);
                        }
                    });
                }
            });
            for (let i in lista) {
                lista_auxiliar[i] = <td style={{ textAlign: "right" }} key={lista_auxiliar[i] + "kjkdfgb"}> {moneyFormat(redondeo(lista_auxiliar[i]) || 0)}</td>
            };
            lista_auxiliar.push(<td style={{ textAlign: "right" }} key={redondeo(total_sub) + "kjbfbbk"}> {moneyFormat(redondeo(total_sub))}</td>);
            lista_.push(<tr key={clasificador + "_123fsdffgddfgdss"} style={{ display: "none"}} className={nombre} >{lista_auxiliar}</tr>);
        });
        return lista_;
    }
    ///<-----------------------------------------------------------------
    const CrearClasificadores = ({clasificadores }) => {
        ...
        clasificadores.forEach(clasificador => {
            ...
            let ident = `tb_${remplazar_espacios_por_guion_bajo(clasificador)}`,
                total_resultado = 0;
            console.info("clasificador : ", clasificador);
            recorrer_lista((clasificador_, establecimiento_) => {
    ...
    const PieDeTotales = () => {
        ...
        lista.forEach(establec => {
            ...
            //TOTAL = TOTAL != "NA" ? redondeo(TOTAL) : TOTAL;
...
const ModalMivimientosPorEstablecimiento = ({ establecimiento }) => {
    const $ORDEN_DATOS_TABLA = ["VENTAS NETAS", "COSTO DE VENTAS", "UTILIDAD EN OPERACION", "GASTOS DE OPERACION"];
    const CeldasTabla = ({ dato, estilo,icono,margen,columnas,mas,pos }) => {
        ...
        let Icono = mas ? <BotonTogle
            identificador={mas}
            poicion={pos}
            />
            : <i className={icono} style={margen}></i>;

        return (
            <th colSpan={columnas} style={estilo}>
                {Icono}
