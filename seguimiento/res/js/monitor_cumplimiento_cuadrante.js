...
//Objetos
var USUARIO = { establecimiento: "Todos", nombre: "", folio :""};
...
llenar_datos_memorio_usuario = function () {
    sessionStorage['id_usuario'] = USUARIO.folio;
    sessionStorage['establecimiento'] = USUARIO.establecimiento;
    sessionStorage['usuario'] = USUARIO.nombre;
}
//manejo de fecha
...
obtener_semana_de_anio= function(){
    SEMANA_ANIO = conexion_ajax("../servicios/manejo_fecha.asmx/retornar_semana_anio", { dia: obtener_fecha('fecha_1 input') });
    //asignar Semana
    $("#selector_semana").val(SEMANA_ANIO.semana)
    $("#selector_anio").val(SEMANA_ANIO.anio)
    //asignar Dias
    $('#fecha_1 input').val(invertir_fecha(SEMANA_ANIO.lunes));
    $('#fecha_2 input').val(invertir_fecha(SEMANA_ANIO.domingo));
...
llenar_tabla_empleado = function (puesto) {
    ...
    var promedios = { Generales_promedios: [] 
